...
pub fn generate_freq<'a>(word_array: &'a Vec<String>) -> HashMap<&'a str, Vec<i32>>{
    let mut delta_encoding: HashMap<&'a str, Vec<i32>> = HashMap::new();
    ...
    for(i, string) in word_array.iter().enumerate() {
        delta_encoding
            .entry(string)
            .and_modify(|vec|{
